...
class AllMissedCallLogsView extends StatefulWidget {
  ...
  @override
  _AllMissedCallLogsViewState createState() {
    return _AllMissedCallLogsViewState();
  }
}
...
class _AllMissedCallLogsViewState extends State<AllMissedCallLogsView>
{
  ...
  void initState()
  {
    ...
    WidgetsBinding.instance?.addPostFrameCallback((_) async {
      DashboardView.ebTransferData.on().listen((event) {
        callLogProvider.doCallLogsApiCall("missed");
      });
    });
  }
  ...
  Widget build(BuildContext context)
  {
    ...
    return ChangeNotifierProvider<CallLogProvider>(
      ...
      {
        this.callLogProvider = CallLogProvider(callLogRepository: callLogRepository);
        callLogProvider.doCallLogsApiCall("missed");
        callLogProvider.doSubscriptionUpdateConversationDetail();
        return callLogProvider;
      },
      child: Consumer<CallLogProvider>(builder: (BuildContext context, CallLogProvider provider, Widget child) {
        ...
        if (callLogProvider.callLogs != null && callLogProvider.callLogs.data != null)
        {
                ...
                children: [
                  RefreshIndicator(
                    ...
                    child: ListView.builder(
                        ...
                        return CallLogVerticalListItem(
                          ...
                          onPressed: () async
                          {
                            ...
                            await Navigator.pushNamed(
                                ...
                                arguments: MessageDetailIntentHolder(
                                  clientName: callLogProvider.callLogs.data[index].recentConversationNodes.clientInfo != null ? callLogProvider.callLogs.data[index].recentConversationNodes.clientInfo.name: Utils.getString("unknown"),
                                  clientPhoneNumber: callLogProvider.callLogs.data[index].recentConversationNodes.clientNumber,
                                  ...
                                  countryId: callLogProvider.callLogs.data[index].recentConversationNodes.clientCountry,
                                  countryFlagUrl: callLogProvider.callLogs.data[index].recentConversationNodes.clientCountryFlag,
                                  isBlocked: callLogProvider.callLogs.data[index].recentConversationNodes.blocked,
                                  lastChatted: callLogProvider.callLogs.data[index].recentConversationNodes.createdAt,
                                  ...
                                  dndMissed: callLogProvider.callLogs.data[index].recentConversationNodes.dndMissed,
                          ...
                          onLongPressed: () {
                            if (isCheckBoxVisible) {
                              setState(() {
                                ...
                                widget
                                    .onSelectAllCheckBoxToggle(isCheckBoxVisible);
                              });
                            } else {
                              setState(() {
                            ...
                            if (value) {
                              ...
                              i++) {
                                setState(() {
                                  ...
                                  true;
                                  toDeleteList.add(
                                      callLogProvider.callLogs.data[i].id);
                                });
                              }
                              setState(() {
                                selectAllCheck = true;
                          ...
                          onCheckBoxTap: (value)
                          {
                            if (value)
                            {
                              setState(()
                              {
                                callLogProvider.callLogs.data[index].check = false;
                                toDeleteList.remove(callLogProvider.callLogs.data[index].id);
                                selectAllCheck = false;
                              });
                            }
                            else
                            {
                              setState(()
                              {
                                callLogProvider.callLogs.data[index].check = true;
                                toDeleteList.add(callLogProvider.callLogs.data[index].id);
                              });
                              if (toDeleteList.length == callLogProvider.callLogs.data.length)
                              {
                                setState(()
                                {
                                  selectAllCheck = true;
                          ...
                          onPinTap: (contactNumber, value)async
                          {
                            ...
                            ContactPinUnpinRequestHolder params = ContactPinUnpinRequestHolder(
                              channel: callLogProvider.getDefaultChannel().id,
                              contact: contactNumber,
                              pinned: value!=null && value?false:true,
                            );
                            Resources<bool> status = await callLogProvider.doContactPinUnpinApiCall(params);
                      ...
                      padding: EdgeInsets.fromLTRB(Dimens.space0, Dimens.space0, Dimens.space0, (kBottomNavigationBarHeight+Dimens.space10).h),
                    ),
                    onRefresh: ()
                    {
                      return callLogProvider.doCallLogsApiCall("missed");
                    },
                  ),
                  Positioned(
                    right: 0,
                      ...
                      child: Offstage(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                              ...
                              color: CustomColors.white),
                          margin: EdgeInsets.fromLTRB(Dimens.space0, Dimens.space0, Dimens.space0, Dimens.space0),
                          padding: EdgeInsets.fromLTRB(Dimens.space10.sp, Dimens.space0, Dimens.space10.sp, Dimens.space0),
                          alignment: Alignment.centerRight,
                          child: RoundedButtonWidgetWithPrefix(
                            titleText: Utils.getString('delete'),
                            width: Dimens.space100.sp,
                            height: Dimens.space40.sp,
                            buttonColor: CustomColors.callDeclineColorLight,
                            textColor: CustomColors.callDeclineColor,
                            corner: Dimens.space8.sp,
                  ...
                  Positioned(
                    ...
                    right: 0,
                    left: 0,
                    child: CustomLinearProgressIndicator(callLogProvider.callLogs.status),
                  ),
                ],
              ),
            );
