...
class IncomingCallNotificationService() : Service() {
    ...
    private fun createNotification(callInvite: CallInvite?, notificationId: Int, channelImportance: Int): Notification {
        val intent: Intent = Intent(this, VoicePlugin::class.java)
        intent.action = Constants.ACTION_INCOMING_CALL_NOTIFICATION
        intent.putExtra(Constants.INCOMING_CALL_NOTIFICATION_ID, notificationId)
        intent.putExtra(Constants.INCOMING_CALL_INVITE, callInvite)
        intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP)
        val pendingIntent: PendingIntent = PendingIntent.getActivity(this, notificationId, intent, PendingIntent.FLAG_UPDATE_CURRENT)
        /*
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            return buildNotification(callInvite?.getFrom().toString() + " is calling.",
                    pendingIntent,
                    extras,
                    callInvite,
                    notificationId,
            ...
            return NotificationCompat.Builder(this,"")
                    ...
                    .setContentText(callInvite?.getFrom().toString() + " is calling.")
                    .setAutoCancel(true)
                    .setExtras(extras)
    ...
    private fun buildNotification(text: String, pendingIntent: PendingIntent, extras: Bundle,
        ...
        val rejectIntent: Intent = Intent(applicationContext, IncomingCallNotificationService::class.java)
        rejectIntent.action = Constants.ACTION_REJECT
        rejectIntent.putExtra(Constants.INCOMING_CALL_INVITE, callInvite)
        rejectIntent.putExtra(Constants.INCOMING_CALL_NOTIFICATION_ID, notificationId)
        val piRejectIntent: PendingIntent = PendingIntent.getService(applicationContext, 0, rejectIntent, PendingIntent.FLAG_UPDATE_CURRENT)
        val acceptIntent: Intent = Intent(applicationContext, IncomingCallNotificationService::class.java)
        acceptIntent.action = Constants.ACTION_ACCEPT
        acceptIntent.putExtra(Constants.INCOMING_CALL_INVITE, callInvite)
        acceptIntent.putExtra(Constants.INCOMING_CALL_NOTIFICATION_ID, notificationId)
        val piAcceptIntent: PendingIntent = PendingIntent.getService(applicationContext, 0, acceptIntent, PendingIntent.FLAG_UPDATE_CURRENT)
    ...
    private fun createChannel(channelImportance: Int): String {
        var callInviteChannel: NotificationChannel = NotificationChannel(Constants.VOICE_CHANNEL_HIGH_IMPORTANCE,
    ...
    private fun accept(callInvite: CallInvite?, notificationId: Int) {
        endForeground()
        val activeCallIntent: Intent = Intent(this, VoicePlugin::class.java)
